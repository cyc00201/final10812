...
class Main: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let controller = segue.destination as?user_interview
        if let Account = sender as? String{
            controller?.Atemp = Account
        }
    }
    ...
    @IBAction func sign_in(_ sender: Any) {
        ...
        //print("login")
        let alertcontroller = UIAlertController(title: "Error", message: "the account has logined!", preferredStyle: .alert)
                       let okAction = UIAlertAction(title: "OK", style: .default, handler: nil)
                       alertcontroller.addAction(okAction)
                       self.present(alertcontroller, animated: true, completion: nil)
